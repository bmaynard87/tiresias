"""Centralized rule registry for Tiresias analysis."""

import re
from collections.abc import Callable
from dataclasses import dataclass

from tiresias.schemas.report import Category, Severity
...
class AnalysisRule:
    ...
    pitfalls: str = ""  # Optional field for common pitfalls (empty for MVP)


def get_all_rules() -> list[AnalysisRule]:
    """
    Build the complete rule set.

    Returns all 12 analysis rules used by the heuristic analyzer.
    Each rule checks for missing or unclear documentation elements.
...
def get_rule_by_id(rule_id: str) -> AnalysisRule | None:
    """
    Lookup a rule by its ID.

    Args:
        rule_id: Rule identifier (e.g., "REQ-001")

    Returns:
        AnalysisRule if found, None otherwise
    """
    rules = get_all_rules()
    for rule in rules:
        if rule.id == rule_id:
...
def list_rule_ids() -> list[tuple[str, str]]:
    """
    Get list of all rule IDs and titles.

    Returns:
        List of (id, title) tuples, sorted by ID
    """
    rules = get_all_rules()
    return sorted([(rule.id, rule.title) for rule in rules], key=lambda x: x[0])
